...
(function($){
	...
	var J2SETTIME="", J2Time=true,J2ROLLING_ANIMATION = {
		...
		start : function(){
			$("#focus-pic div[data-scro='controler'] b,#focus-pic div[data-scro='controler2'] a").mouseover(function(){
		...
		time : function(){
			J2SETTIME = window.setInterval(function(){
				var num = $("#focus-pic div[data-scro='controler'] b[class='down']").index(),
					_list = $("#focus-pic div[data-scro='list'] li");
				_list.eq(num).animate({"left":-$("#focus-pic div[data-scro='list']").width()},"slow",function(){
					$(this).removeAttr("style");	
					$("#focus-pic div[data-scro='controler'] b").removeClass("down").eq(num).addClass("down");
				});	
				num++;
				if(num==_list.size()){
					num=0;
